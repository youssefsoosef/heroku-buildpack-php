...
// TODO: we can throw out any cedar-14 special casing once that's EOL in April 2019
$stacks = [
	1 => '14', // the offset we start with here is relevant for the numbering of footnotes
	'16',
	'18',
];
// TODO: could we dynamically generate these by scanning all repos and somehow evaluating the constraints?
...
$findstacks = function(array $package) use($stacks) {
	if($package['require']) {
		if(isset($package['require']['heroku-sys/cedar'])) {
			return Composer\Semver\Semver::satisfiedBy($stacks, $package['require']['heroku-sys/cedar']);
		} elseif(isset($package['require']['heroku-sys/heroku'])) {
...
while($row = $result->fetchArray(SQLITE3_ASSOC)) {
	$row['data'] = [];
	foreach($series as $serie) {
		$row['data'][$serie] = $filterStackVersions($row, $serie, $stacks, $seriesByStack, function($row, $serie, $stack) { return isset($row["enabled_${serie}_${stack}"]) ? strtr($row["enabled_${serie}_${stack}"], ["0" => "optional", "1" => "&#x2714;"]) : null; });
	}
	$bExtensions[] = $row;
}
