	...
	context "requiring PHP #{series}" do
		before(:all) do
			@app = new_app_with_stack_and_platrepo('test/fixtures/default',
				before_deploy: -> { FileUtils.cp("#{__dir__}/../utils/wait-for-it.sh", FileUtils.pwd); system("composer require --quiet --ignore-platform-reqs php '#{series}.*'") or raise "Failed to require PHP version" }
			)
			@app.deploy
		...
		context "running PHP #{series} and the #{server} web server" do
			...
			combinations.each do | combination |
				...
				context "launching using `#{cmd}'" do
					if combination.value?(false) or cmd.match("broken")
						...
						it "boots" do
							# check if "wait-for-it" exited with status 0, which means the process successfully output the expected message
							expect_exit(expect: :to, code: 0) { @app.run("./wait-for-it.sh 15 'ready for connections' #{cmd}") }
						end
					end
				end
			end
			...
			context "setting concurrency via .user.ini memory_limit" do
				it "calculates concurrency correctly" do
					expect(expect_exit(code: 0) { @app.run("./wait-for-it.sh 15 'ready for connections' heroku-php-#{server} --verbose docroot/") })
				...
				it "always launches at least one worker" do
					expect(expect_exit(code: 0) { @app.run("./wait-for-it.sh 15 'ready for connections' heroku-php-#{server} --verbose docroot/onegig/") })
				...
				it "is only done for a .user.ini directly in the document root" do
					expect(expect_exit(code: 0) { @app.run("./wait-for-it.sh 15 'ready for connections' heroku-php-#{server} --verbose") })
			...
			context "setting concurrency via FPM config memory_limit" do
				it "calculates concurrency correctly" do
					expect(expect_exit(code: 0) { @app.run("./wait-for-it.sh 15 'ready for connections' heroku-php-#{server} --verbose -F conf/fpm.include.conf") })
						 .to match("PHP memory_limit is 32M Bytes")
						.and match("Starting php-fpm with 16 workers...")
				end
				it "always launches at least one worker" do
					expect(expect_exit(code: 0) { @app.run("./wait-for-it.sh 15 'ready for connections' heroku-php-#{server} --verbose -F conf/fpm.onegig.conf") })
						 .to match("PHP memory_limit is 1024M Bytes")
						.and match("Starting php-fpm with 1 workers...")
				end
				it "takes precedence over a .user.ini memory_limit" do
					expect(expect_exit(code: 0) { @app.run("./wait-for-it.sh 15 'ready for connections' heroku-php-#{server} --verbose -F conf/fpm.include.conf docroot/onegig/") })
						 .to match("PHP memory_limit is 32M Bytes")
						.and match("Starting php-fpm with 16 workers...")
				end
			end
			...
			context "setting WEB_CONCURRENCY explicitly" do
				it "uses the explicit value" do
					expect(expect_exit(code: 0) { @app.run("./wait-for-it.sh 15 'ready for connections' heroku-php-#{server} --verbose", :heroku => {:env => "WEB_CONCURRENCY=22"}) })
				...
				it "overrides a .user.ini memory_limit" do
					expect(expect_exit(code: 0) { @app.run("./wait-for-it.sh 15 'ready for connections' heroku-php-#{server} --verbose docroot/onegig/", :heroku => {:env => "WEB_CONCURRENCY=22"}) })
						 .to match("\\$WEB_CONCURRENCY env var is set, skipping automatic calculation")
						.and match("Starting php-fpm with 22 workers...")
				end
				it "overrides an FPM config memory_limit" do
					expect(expect_exit(code: 0) { @app.run("./wait-for-it.sh 15 'ready for connections' heroku-php-#{server} --verbose -F conf/fpm.onegig.conf", :heroku => {:env => "WEB_CONCURRENCY=22"}) })
						 .to match("\\$WEB_CONCURRENCY env var is set, skipping automatic calculation")
						.and match("Starting php-fpm with 22 workers...")
				end
			end
			...
			context "running on a Performance-L dyno" do
				it "restricts the app to 6 GB of RAM", :if => series < "7.4" do
					expect(expect_exit(code: 0) { @app.run("./wait-for-it.sh 15 'ready for connections' heroku-php-#{server} --verbose", :heroku => {:size => "Performance-L"}) })
