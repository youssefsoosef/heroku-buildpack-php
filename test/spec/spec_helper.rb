...
# override run to use --exit-code
module Hatchet
	class App
		def run(cmd_type, command = nil, options = {}, &block)
			command        = cmd_type.to_s if command.nil?
			heroku_options = (options.delete(:heroku) || {}).map {|k,v| "--#{k.to_s.shellescape}".concat(v ? "=#{v.to_s.shellescape}" : "")}.join(" ")
			heroku_command = "heroku run -a #{name} #{ heroku_options } --exit-code -- #{command.to_s.shellescape}"
			bundle_exec do
				if block_given?
