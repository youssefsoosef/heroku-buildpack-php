...
print_help() {
	cat >&2 <<-EOF
		...
		Usage:
		  wait-for-it [options] <DURATION> <TEXT_TO_MATCH> <COMMAND...>
		...
		The exit status of 'wait-for-it' will be the exit status of 'timeout' if the
		...
		Options:
		  -s, --silent       Do not relay COMMAND's stdout+stderr to stderr.
		  -o, --only-stdout  Do not capture COMMAND's stderr for matching. 
		...
		wait-for-it 15 "ready for connections" start-web-server.sh | \
		...
		have a non-zero exit status if the 'wait-for-it' invocation timed out or did
